...
DO INSTEAD (
  INSERT 
  INTO license
    (id, company, effective_date, audit_user, audit_action, audit_date)
    SELECT * FROM (  
      SELECT
        nextval('global_id'),
        NEW.license_company,
    ...
    WHERE 
      a.license_company NOT IN (select company FROM license);
  
  INSERT
  INTO movie 
     (id, title, license_owner, audit_user, audit_action, audit_date)
  SELECT nextval('global_id'),
         ...
         now()
   FROM license l
   WHERE l.company = new.license_company
   LIMIT 1;
);
